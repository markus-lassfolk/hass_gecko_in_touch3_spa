...
@pytest.mark.asyncio
async def test_pending_target_temperature_holds_against_stale_report(mock_hass):
    """UI must not rubberband while MQTT ``reported`` still carries the old setpoint."""
    hass_stub = MagicMock()
    publish = MagicMock()
    transporter = MagicMock()
    transporter.publish_desired_state = publish
    ...
    with (
        ...
        await ent.async_set_temperature(temperature=31.5)
        zone.target_temperature = 29.0
        ent._update_from_zone()
        assert ent._attr_target_temperature == 31.5

        zone.target_temperature = 31.5
        ent._update_from_zone()
        assert abs(float(ent._attr_target_temperature) - 31.5) < 0.01


@pytest.mark.asyncio
async def test_pending_target_temperature_expires_after_grace(mock_hass):
    hass_stub = MagicMock()
    publish = MagicMock()
    transporter = MagicMock()
    transporter.publish_desired_state = publish
    ...
    with (
        ...
        mono_at[0] = 0.0
        await ent.async_set_temperature(temperature=33.0)
        assert ent._attr_target_temperature == 33.0
        zone.target_temperature = 29.0
        ent._update_from_zone()
        assert ent._attr_target_temperature == 33.0
        mono_at[0] = GeckoClimate._PENDING_TARGET_GRACE_S + 1.0
        ent._update_from_zone()
        assert ent._attr_target_temperature == 29.0
